# BLIND PREDICTION TASK — F2r1__policy_specific_mean__age_band__c01
...
All 7 items behind the scored outcome `policy_specific_mean`. Predict each one
separately: they are
...
Write one JSON object of exactly this shape, and nothing else, to:
  ...
  generation/runs/raw/F2r1__policy_specific_mean__age_band__c01.json
...
```json
{
  "task_id": "F2r1__policy_specific_mean__age_band__c01",
...
When the file is written, reply with only the number of values it holds (476).
