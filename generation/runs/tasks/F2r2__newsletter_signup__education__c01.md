# BLIND PREDICTION TASK — F2r2__newsletter_signup__education__c01
...
The single item behind the scored outcome `newsletter_signup`. Predict each
one separately: they are
asked as separate survey questions.

  newsletter
      Did you subscribe to the "Talking Climate" newsletter on the previous page? (recoded 1/0 in cleaning)
      scale: Raw Qualtrics: 1 = Yes, 2 = No (recoded to 1/0 in cleaning). Refers to an earlier newsletter-offer page; see questionnaire.txt for the cross-page dependency.
      give:  percentage of the group who subscribed (0-100, 1 decimal)
...
Write one JSON object of exactly this shape, and nothing else, to:
  ...
  generation/runs/raw/F2r2__newsletter_signup__education__c01.json
...
```json
{
  "task_id": "F2r2__newsletter_signup__education__c01",
  "model_id": "<your exact model id>",
  "read_check": "<first four words of C06>",
  "values": {
    "L1": {"newsletter": {"C00": 12.5, "C16": 12.5, "C15": 12.5, ...}},
    "L2": {"newsletter": {"C00": 12.5, "C16": 12.5, "C15": 12.5, ...}},
    ...
  }
}
```
...
* Every group code, every item name and every condition code listed above must
  ...
  for numbers.
* Values are percentages, 0 to 100, one decimal: the share of the group that
  signed up.

When the file is written, reply with only the number of values it holds (102).
